...
def dl_video():
    ...
    try:
        ...
        if f_type == "MP4":
            stream = yt.streams.get_highest_resolution().download(output_path=f"{os.environ['UserProfile']}/Desktop")
            status_label.configure(text=f"Sucessfully Downloaded {yt.title}", text_color = "white", fg_color="green")
        elif f_type == "MP3":
            stream = yt.streams.get_audio_only().download(output_path=f"{os.environ['UserProfile']}/Desktop")
            status_label.configure(text=f"Sucessfully Downloaded {yt.title}", text_color = "white", fg_color="green")
            clip = AudioFileClip(stream)
            clip.write_audiofile(stream[:-4] + ".mp3")
            clip.close()
            os.remove(stream)
...
filetype_combobox.set("File Type")
